...
public class ScimResource implements IResourceModifier, IBulkIdTarget {
     ...
     * @param bulkIdResolver An {@link IBulkIdResolver} used to resolve identifiers during bulk operations
     * @param container      A String identifying the resource container where the object is from or to be stored (e.g.
     *                       Users, Groups). Used to lookup ResourceType and Schema
     * @throws SchemaException Thrown when object parsed violates SCIM Schema
     * @throws ParseException  Thrown when a JSON parsing error occurs
     * @throws ScimException   Thrown due to internal SCIM error (TBD)
     */
    ...
    /**
     * @return The {@link Schema} for the main content of the ScimResource.
     */
    ...
    /**
     * Calculate the MD5 digest hash of the resource based on its JSON representation.
    ...
    /**
     * Returns the attribute definition that corresponds to an attribute within the resource.
     * @param name The name or full name path of the attribute
     * @param ctx  The request ctx (used to default the resource type)
    ...
    /**
     * Return a value based on the attribute definition (which includes the attributes name, path, and schema). This
     * procedure automatically checks core and extended attributes to locate the correct value.
     * @param attr An attribute (e.g. from Schemas) definition
     * @return A Value representing the corresponding attribute value if defined (or null).
     */
    ...
    /**
     * In preparation for a PUT, remove all the attributes that will be replaced (which are readWrite attributes).
     */
    ...
    /**
     * Copies the current ScimResource and returns a new ScimResource object. Include a {@link RequestCtx} to do
     * attribute filtering.
     * @param requestCtx A RequestCtx object which may be used to filter included/excluded attributes and security
     *                   filtering.
     * @return a New ScimResource based on a JsonNode copied result.
     * @throws SchemaException Thrown when an invalid value is parsed compared to the defined attribute
     * @throws ParseException  Thrown due to JSON parsing error
     */
     ...
     * @return A set of Attribute definitions that are present in the current resource. Includes core and extension
     * attributes.
     */
