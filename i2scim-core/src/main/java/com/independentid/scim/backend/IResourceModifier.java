...
/**
 * IResourceModifier defines an interface to a <code>ScimResource</code> which can be used to update or replace a
 * resource.
 * @author pjdhunt
 */
public interface IResourceModifier {
    ...
    /**
     * Accepts a SCIM Patch request (similar to JSON Patch) and processes against the current ScimResource. If all
     * operations succeed, the current scim object is modified (but not yet persisted) and true is returned. Processing
     * is subject to attribute field mutability.
    ...
    /**
     * Attempts to replace the current resource (subject to attribute mutability) with the specified parsed resource
     * object.  Non-mutable fields are left untouched.
     ...
     * @throws PreconditionFailException occurs if the precondition fails
     */
    boolean checkModPreConditionFail(RequestCtx ctx) throws PreconditionFailException;

    /**
     * Checks the <code>RequestCtx</code> for If-Not-Match and If-Modified-Since preconditions with the current resource
     * for a match. If not matched, false is returned.
     * @param ctx A RequestCtx containing the request headers
     * @return true if preconditions fail
     * @throws PreconditionFailException occurs if the precondition fails
     */
