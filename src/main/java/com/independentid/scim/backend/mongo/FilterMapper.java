...
public class FilterMapper {
		...
		case Attribute.TYPE_String: {
			...
			case AttributeFilter.FILTEROP_EQ: {
				if (!filter.getAttribute().getCaseExact()) {
					...
					*/
					...
					buf.append('^').append(filter.asString()).append('$');
					Document regEx = new Document("$regex", buf.toString());
			...
			case AttributeFilter.FILTEROP_NE:
				if (!filter.getAttribute().getCaseExact()) {
					...
					if (negate) {
						/*
						Document not = new Document("$not", //$NON-NLS-1$
								new Document("$ne", filter.getValue())); //$NON-NLS-1$
						obj.put(aname, not);
						*/
						// Not NE is EQ
						obj.put(aname, regex);
	...
	public static Document mapFilter(PrecedenceFilter filter, boolean negate)
		...
		Document cobj = FilterMapper.mapFilter(filter.getChildFilter(),
				(negate ? !filter.isNot() : filter.isNot()));
