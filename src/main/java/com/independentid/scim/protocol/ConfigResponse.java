...
import com.independentid.scim.server.ForbiddenException;
import com.independentid.scim.server.NotFoundException;
import com.independentid.scim.server.ScimException;

/**
 * ConfigResponse acts as a special response provider by acting as a virtual
 * endpoint for Configuration endpoints (e.g. /ResourceTypes, /Schemas, or
...
public class ConfigResponse extends ListResponse {
	...
	private static ConfigMgr scfg = ConfigMgr.getInstance();
	 ...
	 * @param ctx
	 */
	public ConfigResponse(RequestCtx ctx) {
		super(ctx);
		...
		case ScimParams.PATH_SERV_PROV_CFG:
			...
			} catch (SchemaException | ParseException | ScimException | IOException e) {
		...
		case ScimParams.PATH_TYPE_SCHEMAS:
			...
			if (ctx.getPathId() != null) {
				...
				// Return all schemas
				Iterator<Schema> siter = scfg.getSchemas()
						.iterator();
				
				while (siter.hasNext()) {
					Schema sch = siter.next();
		...
		case ScimParams.PATH_TYPE_RESOURCETYPE:
			...
			if (ctx.getPathId() != null) {
				...
				Iterator<ResourceType> iter = scfg.getResourceTypes()
						.iterator();
				
				while (iter.hasNext()) {
					ResourceType rt = iter.next();
	...
	/**
	 * @param path A String containing the path to be checked. Routine checks the SCIM container level path element only.
